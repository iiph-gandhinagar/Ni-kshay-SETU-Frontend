...
const ProgressLine = ({
  ...
  ], boxShadow, customClass, customPercentage, customProgressLineClass
}) => {
  ...
  return (
    <div className={`d-flex align-items-center custom-progress-bar pt-2 ${customClass ? customClass : ""}`}>
      <div className="progressLabel">{label}</div>
      <div
        className={customProgressLineClass ? customProgressLineClass : "progressVisualFull"}
        // to change the background color dynamically
        style={{
          backgroundColor, boxShadow: boxShadow
        }}
      >
      ...
      <Text variant="Nunito18title" sx={{ color: "Grey_2" }} className="me-2">25%</Text>
}
    </div>
  );
};
